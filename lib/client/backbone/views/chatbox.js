LocalyBackbone.Views.Chatbox = LocalyBackbone.LocalyView.extend({
  ...
  global_events: {
    ...
    "swapActiveHoodButton": "swapActiveHoodButton",
    "swapRoom": "swapRoom",
    "scrollToChatboxBottom": "scrollToChatboxBottom"
  },
  ...
  displayMessage: function(msgType, msg, userSocketID, msgDestination, options) {
    ...
    var timestamp = options ? options.timestamp : "";
    ...
    this.scrollToChatboxBottom($chatboxUl.height());
  },

  scrollToChatboxBottom: function(height) {
    $("div#ul-wrapper").stop().animate({ scrollTop: height }, "fast");
  },
  ...
  addIndividualButton: function(socketID, username) {
    var $individualButton = this._$individualButton(socketID);

    if ($individualButton.length === 0) {
      var renderedIndividualButton = window.JST['individual-button']({socketID: socketID, username: username});
      $("#chatbox-button-wrapper").append(renderedIndividualButton);
      if (Localy.inIndividual(socketID)) this._addBlue(this._$individualButton(socketID));
    } else {
      if (!Localy.inIndividual(socketID)) {
        this.showBadge($individualButton.children(".notification-badge"));
      }
    }
  },

  swapActiveHoodButton: function(hood) {
    if (this._$hoodButton(hood).length === 0) {
      var hoodButton = window.JST['other-hood-button']({hood: hood});
      $("#chatbox-button-wrapper").append(hoodButton);      
      this._findOrCreateChatbox(hood).empty();
      window.socket.emit('get chats from room', hood);
    }

    this._addBlue(this._$hoodButton(hood));
    var $chatboxUl = this._findOrCreateChatbox(hood);
    this.scrollToChatboxBottom($chatboxUl.height());
  },
  ...
  _findOrCreateChatbox: function(type, options) {
    ...
    if ($chatboxUl.length === 0) {
      ...
      this.$containerEl.find("#ul-wrapper").append($chatboxUl);
    }
  ...
  _swapActiveUl: function(type, options) {
    $("#chatbox-wrapper").find('ul.active').removeClass("active");
