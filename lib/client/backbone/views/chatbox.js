LocalyBackbone.Views.Chatbox = LocalyBackbone.LocalyView.extend({
  ...
  submitMessage: function(event) {
    ...
    console.log("in local?", Localy.inLocal())
    console.log("in hood?", Localy.inHood())
    if (Localy.inLocal()) {
  ...
  displayMessage: function(msg, userSocketID, type, bounds) {
    ...
    this.updateButtonBadge(type, bounds);

    //Set the correct chatbox
    var $chatboxUl = this._findOrCreateChatbox(type, bounds);
  ...
  updateButtonBadge: function(type, bounds) {
    ...
    switch(type) {
      ...
      case window.placename:
        if (!Localy.inHood()) this.showBadge($hoodbadge);
        break;
  ...
  _findOrCreateChatbox: function(type, bounds) {
    console.log("finding chatbox", type, bounds)
  ...
  _swapActiveUl: function(type, bounds) {
    ...
    return this._findOrCreateChatbox(type, bounds).addClass("active");
  },
