...
window.Localy = {
	...
	displayMessage: function(msg, userID, type, bounds){
		msg = Helper.replaceHTMLEntities(msg);
	...
	handleType: function(type, bounds){
		switch(type){
			...
			case "remote":
				...
				var $remotebutton = $(".remote-button[lat='" + lat + "'][lon='" + lon + "']");				
				
				if ($remotebutton.length === 0) {
					Localy.addRemoteButton(lat, lon);
				} else {
					if (!Localy._inRemote(lat, lon)){
						var $remotebadge = $remotebutton.children(".notification-badge");
	...
	addRemoteButton: function(lat, lon){
		...
		var remoteButton = "<div "
												...
												+ ", "
												+ lon.toFixed(2)
												+ "<span style='display: " + spanIsVisible + "' class='notification-badge'>1</span></div>";
