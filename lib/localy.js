...
$(document).ready(function(){
  ...
  $('form#chat-form').submit(function(event){
	    	...
	    	socket.emit('radius message', $('input#chat-input').val(), socket.io.engine.id, window.lat, window.lon);
			} else if (window.remoteCircle !== undefined) {
				var lat = window.getLatLng().lat;
				var lon = window.getLatLng().lon;
				
	    	socket.emit('radius message', $('input#chat-input').val(), socket.io.engine.id, lat, lon);
			} else {
