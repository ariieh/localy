...
$(document).ready(function(){
    ...
    if(event.keyCode == 13){
  	  ...
  	  Locator.getLocation(name, function(position, name){
				...
				socket.emit('load marker', position, name, selfID, that.placeName);
  ...
  $('form#chat-form').submit(function(e){
			e.preventDefault();
    	socket.emit('chat message', $('input#chat-input').val(), socket.io.engine.id, this.placeName);
			$('input#chat-input').val('');
  });
	
	/* Socket listeners */
	socket.on('load map', function(serverMarkerData, callback){		
