...
window.Localy = {
	...
	handleType: function(type, options){
		switch(type){
			...
			case "remote":
				...
				if (!document.getElementById(lat + "," + lon)) Localy.addRemoteButton(options.lat, options.lon);
				break;
			}
	},
	...
	addRemoteButton: function(){
		var remoteButton = "<div "
												+ "id='"
												+ lat + "," + lon
												+ "' class='nifty-box chatroom-button'>Remote: "
												+ lat.toFixed(2)
												+ ", "
												+ lon.toFixed(2)
												+ "<span class='notification-badge'>0</span></div>";

		$chatbox.append(remoteButton);
	},
...
$(document).ready(function(){
  ...
  $('form#chat-form').submit(function(event){
	    	...
	    	socket.emit('radius message', $('input#chat-input').val(), socket.io.engine.id, "local", bounds);
				...
				var bounds = MathLib.radiusBounds(window.remoteCircle.getLatLng().lat, window.remoteCircle.getLatLng().lon);
