...
window.Localy = {
	...
	joinRemote: function(lat, lon) {
		...
		Localy._swapActiveUl(Localy._typeClass("remote", bounds));
	},
	...
	_typeClass: function(type, bounds) {
		return type === "remote" ? bounds.degree.lat + "-" + bounds.degree.lon : type;
	},
	...
	_swapActiveUl: function(type) {
		$chatbox.children('ul.active').removeClass("active");
		var $typeChatboxUl = $("#chatbox").children("ul." + type);

		if ($typeChatboxUl.length > 0) {
			$typeChatboxUl.addClass("active");
		} else {
			$typeChatboxUl = $("<ul class='active " + type + "'></ul>");
			$chatbox.append($typeChatboxUl);
		}

		return $typeChatboxUl;
	}
}
