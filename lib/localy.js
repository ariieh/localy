  ...
  $('form#chat-form').submit(function(event){
	...
	$('#chatbox #local-button').click(function(event){
		...
		if (window.localCircle !== undefined){
			map.removeLayer(window.localCircle);
			window.localCircle = undefined;
			window.localChat = false;
		} else {
			window.localCircle = L.circle([window.lat, window.lon], 32.1869, {
				weight: 1,
				opacity: 0.5,
				fillOpacity: 0.4,
				color: '#3498db'
			});
			window.localCircle.addTo(map);
			window.localChat = true;
		}
		// $chatbox.children("ul").children().fadeOut(100).promise().then(function(){
		// 	$chatbox.children("ul").empty();
		// })
	});
