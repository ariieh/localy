...
window.Locator = {
    ...
    navigator.geolocation.getCurrentPosition(function(position){
			...
			var places = leafletPip.pointInLayer([that.longitude, that.latitude], geojson);
			
			if (places.length > 0){
				that.placeName = places[0].feature.properties.NTAName;
			} else {
				that.placeName = "";
			}
			
			Localy.swapChatTitle(that.placeName);
